...
<template>
      <div v-for="(item, index) in projects" class="grid grid-cols-1 md:grid-cols-2 items-center gap-10 md:gap-5 md:py-10">
          <!-- Text -->
          <div class="space-y-2 text-left max-w-md justify-self-center md:justify-self-end order-2 md:order-1">
        ...
        :pt="{
            root:{
                class: 'w-full h-px bg-gray-900 order-3 md:bg-white'
            }
        }"
          />
            ...
            <img :src="item.image_src" :alt="item.image_alt" class="
                <!-- mx-auto object-center h-48 w-96 md:h-72 md:w-120 object-cover rounded-xl shadow-lg -->
            mx-auto h-48 w-full max-w-md md:h-72 md:w-[30rem] object-cover rounded-xl shadow-md transition-transform duration-300 ease-in-out hover:scale-105 hover:shadow-xl order-1 md:order-2
            ">
        </div>
</template>
